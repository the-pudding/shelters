...
function readerSelNest({ dogs, counts }) {
  ...
  if (readerState !== lastReaderState) {
    ...
    const test = $toggle.selectAll('option').property('selected', function(d) {
  ...
  if (noDogsMoved) {
    ...
    $warning
      .select('.condition')
      .text(selToggle === 'imports' ? 'imported from' : 'exported');
  }
...
function nestDogs(loc, toggle) {
  ...
  const filteredImports = exportedDogs.filter(
    d => d.final_state === readerState
  );
  const filteredExports = exportedDogs.filter(
    d => d.original_state === readerState
  );
...
function init(loc) {
  return new Promise((resolve, reject) => {
    const files = ['exampleDogs.json', 'exportedDogs.csv'];
